...
- **Netlify**：运行 Next.js 页面和 API
- **Supabase PostgreSQL**：保存用户、密码 Hash、作品、资料、验证码与限流记录
- **Supabase Storage（Private）**：保存作品图片、视频与本人图片
- **腾讯云 SES API**：发送注册 / 找回密码验证码

> Supabase 只承担数据库和私有媒体存储，本项目**不使用 Supabase Auth**。项目使用 scrypt 密码 Hash、HttpOnly Session、authVersion、邮箱验证码和腾讯云 SES 验证流程。
...
图片和视频不再经过 Netlify Function 传输完整文件：

1. 登录用户向站内 API 请求一次性 Supabase Signed Upload URL。
2. 浏览器把媒体文件直接上传到 Private Storage 的临时目录。
3. 服务端重新读取临时对象，检查 Magic Bytes、真实格式、尺寸和文件大小。
4. JPG / PNG / WEBP 经 Sharp 重编码并剥离元数据；GIF 保留动画但仍验证格式与尺寸。
5. 视频验证 MP4 / WEBM / MOV / M4V 真实格式与 200MB 大小上限。
6. 合格媒体移动到正式 `media/` 路径，临时对象删除。
7. 查看媒体时先经过网站权限检查，再签发短时 Supabase Signed URL。

单张图片最大 15MB；单个视频最大 200MB；作品图片最多 12 张、视频最多 4 个；本人介绍最多 3 张图片。
...
旧的 Vinext / Cloudflare / 腾讯 Docker 文件暂时保留在仓库中，但不再参与当前生产构建；待 Netlify + Supabase 真机验收通过后再做最终依赖清理。
