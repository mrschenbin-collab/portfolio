...
- 新增 `supabase/migrations/002_registration_capacity.sql`，用数据库事务锁保证全站最多 20 个正式账号。
...
- 原有 scrypt 密码 Hash、HttpOnly Session、authVersion、验证码 / 登录限流与腾讯云 SES 保留。
...
4. 部署并测试注册、登录、重置密码、作品 CRUD、5MB+ 图片、两个账号权限隔离。
5. 全部通过后，再清理旧 Vinext / Cloudflare / Drizzle / Tencent Docker 依赖与遗留文件。
