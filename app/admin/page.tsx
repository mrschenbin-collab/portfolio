...
export const metadata: Metadata = { title: "作品后台", robots: { index: false, follow: false } };
...
export default async function AdminPage() {
  ...
  return <main className="admin-page section-pad">
    <header className="admin-hero"><div><p className="eyebrow">作品后台</p><h1>作品提交</h1></div><div className="admin-account"><span>{user.displayName}</span><small>{user.email}</small><form action="/api/auth/logout" method="post"><button type="submit" className="logout-button">退出登录</button></form></div></header>
